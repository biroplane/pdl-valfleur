...
<template>
  <main>
    ...
    <div class="grid md:grid-cols-2 gap-6 md:gap-16 pb-12 md:py-24">
      <NuxtImg format="webp" src="/chi_siamo.jpg" class=" w-full max-h-[50vh] object-cover md:object-contain md:object-right md:rounded-lg" />
    ...
    <section class=" py-12 bg-primary-50 flex items-center justify-center px-8 md:px-0">
      <div class="">
        ...
        <ul class="grid md:grid-cols-3 max-w-6xl  w-full mx-auto gap-6">
          <li v-for="(punto, p) in puntidiforza" :key="p" class="card">
            <NuxtImg format="webp" :src="punto.img" class="w-full h-64 aspect-square object-cover rounded-t-md" />
          ...
          <template #item="{ item, index }">
            <div class="w-full flex flex-col md:flex-row justify-between px-4 py-8 gap-4 md:gap-24">
              ...
              <div class=" text-left flex-grow" :class="{ 'md:-order-first': index % 2 === 1 }">
                <h3 class="text-xl md:text-3xl mb-8 font-body text-primary-500">
                  {{ item.name }}
                </h3>
                ...
                <NuxtLink :to="`/servizi#${item.name}`" class="mt-12 font-bold underline inline-block">
                  Scopri di più
                </NuxtLink>
              </div>
            </div>
